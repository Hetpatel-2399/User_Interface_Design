...
namespace ChessLibrary
{
	...
	public class Rules
	{
		private Board s_Board;	// store a reference to the game board
		private Game s_Game;	// store a reference to the current game
		...
		public int DoMove(Move move)
		{
            ...
            ArrayList LegalMoves = GetLegalMoves(s_Board[move.StartCell]);

            if (!LegalMoves.Contains(s_Board[move.EndCell]))	// Not a legal move
		...
		{
			s_Board[move.StartCell].piece.Moves++;	// incremenet moves
			s_Board[move.EndCell].piece = s_Board[move.StartCell].piece;		// Move object at the destination
			s_Board[move.StartCell].piece = new Piece(Piece.PieceType.Empty);	// Empty the source location
		}
		...
		private void DoTowerMove(Move move)
		{
			...
			{
				Cell rockcell = s_Board.RightCell(move.EndCell);
				Move newmove = new Move(rockcell,s_Board.LeftCell(move.EndCell)); // create the move for rock
				DoNormalMove(newmove); // Move the rock
			}
			else
			{
				// Move to the left side
				Cell rockcell = s_Board.LeftCell(move.EndCell);
                rockcell = s_Board.LeftCell(rockcell);
				Move newmove = new Move(rockcell,s_Board.RightCell(move.EndCell)); // create the move for rock
				DoNormalMove(newmove); // Move the rock
			}
		}
		...
		private void DoPromoMove(Move move)
		{
			...
			if (move.PromoPiece==null)
				s_Board[move.EndCell].piece = new Piece(Piece.PieceType.Queen, s_Board[move.EndCell].piece.Side);	// Set the end cell to queen
			else
				s_Board[move.EndCell].piece = move.PromoPiece;
		}
		...
		private void DoEnPassantMove(Move move)
		{
			...
			if (move.StartCell.piece.Side.isWhite())	// white piece is moving
				EnPassantCell = s_Board.BottomCell(move.EndCell);	// Get the cell under target position
			else
				EnPassantCell = s_Board.TopCell(move.EndCell);	// Get the cell under target position
		...
		public void UndoMove(Move move)
		{
			...
			if (move.Type == Move.MoveType.TowerMove)
			{
				...
				if (move.EndCell.col > move.StartCell.col) // moving right
				{
					...
					Cell source = s_Board.LeftCell(move.EndCell);	// Get the new position of the rock
					Cell target = s_Board[move.StartCell.row, 8];	// Get the rook orignal position

					s_Board[source].piece.Moves--;	// decrement moves
					s_Board[target].piece = s_Board[source].piece;		// Move object at the destination
					s_Board[source].piece = new Piece(Piece.PieceType.Empty);	// Empty the source location	
				}
				else	// Moving Left
				{
					// Now move the rook back to it's orignal position
					Cell source = s_Board.RightCell(move.EndCell);	// Get the new position of the rock
					Cell target = s_Board[move.StartCell.row, 1];	// Get the rook orignal position
		...
		{
			s_Board[move.EndCell].piece = move.CapturedPiece;		// Move object at the destination
			s_Board[move.StartCell].piece = move.Piece;	// Empty the source location
			s_Board[move.StartCell].piece.Moves--;	// decrement moves
		}
        ...
        public bool IsUnderCheck(Side.SideType PlayerSide)
		{
			...
			ArrayList OwnerCells = s_Board.GetSideCell(PlayerSide);

			// loop all the owner squars and get his king cell
			foreach (string CellName in OwnerCells)
			{
				if (s_Board[CellName].piece.Type == Piece.PieceType.King )
				{
					OwnerKingCell = s_Board[CellName]; // store the enemy cell position
					break;	// break the loop
				}
			}

			// Loop all the enemy squars and get their possible moves
			ArrayList EnemyCells = s_Board.GetSideCell((new Side(PlayerSide)).Enemy());
			foreach (string CellName in EnemyCells)
			{
				ArrayList moves = GetPossibleMoves(s_Board[CellName]);	// Get the moves for the enemy piece
        ...
        private int GetCountOfPossibleMoves(Side.SideType PlayerSide)
		{
			...
			ArrayList PlayerCells = s_Board.GetSideCell(PlayerSide);
			foreach (string CellName in PlayerCells)
			{
				ArrayList moves = GetLegalMoves(s_Board[CellName]);	// Get all the legal moves for the owner piece
		...
		public ArrayList GenerateAllLegalMoves(Side PlayerSide)
		{
			...
			ArrayList PlayerCells = s_Board.GetSideCell(PlayerSide.type);
			...
			foreach (string CellName in PlayerCells)
			{
				...
				{
					move = new Move(s_Board[CellName], dest);
		...
		public ArrayList GenerateGoodCaptureMoves(Side PlayerSide)
		{
			...
			foreach (string CellName in PlayerCells)
			{
				...
				if (s_Board[CellName].piece.GetWeight()> 100)
				{
					ArrayList moves = GetLegalMoves(s_Board[CellName]);	// Get all the legal moves for the owner piece
		...
		private Move LastMoveWasPawnBegin()
		{
			...
			Move lastmove = s_Game.GetLastMove();
		...
		private void GetPawnMoves(Cell source, ArrayList moves)
		{
			...
			if (source.piece.Side.isWhite())
			{
				...
				newcell = s_Board.TopCell(source);	
				...
				{
					newcell = s_Board.TopCell(newcell);	
			...
			else
			{
				...
				newcell = s_Board.BottomCell(source);	
				...
				if (newcell!=null && newcell.IsEmpty())
				{
					newcell = s_Board.BottomCell(newcell);	
				...
				newcell = s_Board.BottomLeftCell(source);	
				...
				newcell = s_Board.BottomRightCell(source);	
				if (newcell!=null && newcell.IsOwnedByEnemy(source)) // Bottom cell is available for the move
					moves.Add(newcell);
		...
		private void GetKnightMoves(Cell source, ArrayList moves)
		{
			...
			if (newcell!=null)
			{
				newcell = s_Board.BottomLeftCell(newcell);
				// target cell is empty or is owned by the enemy piece
				if (newcell!=null && !newcell.IsOwned(source) ) 
					moves.Add(newcell);

				newcell = s_Board.BottomCell(source);
				newcell = s_Board.BottomRightCell(newcell);
				// target cell is empty or is owned by the enemy piece
				if (newcell!=null && !newcell.IsOwned(source) ) 
					moves.Add(newcell);
			}
			// Now check 2nd Left Top and bottom cells
			newcell = s_Board.LeftCell(source);
			if (newcell!=null)
			{
				newcell = s_Board.TopLeftCell(newcell);
				// target cell is empty or is owned by the enemy piece
			...
			newcell = s_Board.RightCell(source);
			if (newcell!=null)
			{
				newcell = s_Board.TopRightCell(newcell);
				// target cell is empty or is owned by the enemy piece
		...
		private void GetRookMoves(Cell source, ArrayList moves)
		{
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.TopCell(newcell); // keep moving in the top direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.LeftCell(newcell); // keep moving in the left direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.RightCell(newcell); // keep moving in the right direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.BottomCell(newcell); // keep moving in the bottom direction
			}
		}
		...
		private void GetBishopMoves(Cell source, ArrayList moves)
		{
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.TopLeftCell(newcell); // keep moving in the top-left direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.TopRightCell(newcell); // keep moving in the top-right direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.BottomLeftCell(newcell); // keep moving in the bottom-left direction
			}
			...
			while (newcell!=null)	// move as long as cell is available in this direction
			{
				...
				newcell = s_Board.BottomRightCell(newcell); // keep moving in the bottom-right direction
			}
		}
		...
		private void GetKingMoves(Cell source, ArrayList moves)
		{
			...
			if (s_Board[source].piece.Moves == 0)
			{
				...
				if (newcell!=null && newcell.IsEmpty())	// cell is empty
				{
					...
					{
                        newcell = s_Board.RightCell(newcell);
						if (newcell!=null && newcell.IsEmpty())	// cell is empty
						{
				...
				if (newcell!=null && newcell.IsEmpty())	// cell is empty
				{
					if (!CauseCheck(new Move(source, newcell))) // Inbetween cell is not under check
					{
                        newcell = s_Board.LeftCell(newcell);
						if (newcell!=null && newcell.IsEmpty())	// cell is empty
						{
							CastlingTarget = newcell;	// This will be the king destination position
                            newcell = s_Board.LeftCell(newcell);
        ...
        public int AnalyzeBoard(Side.SideType PlayerSide)
		{
			...
			{
				Score+=s_Board[ChessCell].piece.GetWeight();
			}
